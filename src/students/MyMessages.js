...
export default function MyMessages() {
  ...
  const loadMessages = async (studentId) => {
    try {
      ...
      const receivedResult = await axios.get(`http://localhost:8080/messages/student/${studentId}`);
      setReceivedMessages(receivedResult.data);

      // Optionally, fetch sent messages if the endpoint exists
      const sentResult = await axios.get(`http://localhost:8080/messages/sent/student/${studentId}`);
